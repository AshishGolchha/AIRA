    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <GlassCard className="lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">Top Portfolio Holdings</h3>
              </div>
              <Link to="/app/portfolio" className="text-xs text-brand-400 hover:text-brand-300 font-medium flex items-center gap-1">
            ...
            {portfolio.top_holdings.length === 0 ? (
              <div className="text-center py-10 text-slate-500 text-xs">
                ...
                <Link to="/app/portfolio" className="text-brand-400 underline">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="text-[11px] text-slate-400 uppercase font-semibold border-b border-border-subtle">
                    <tr>
                  ...
                  <tbody className="divide-y divide-border-subtle/50">
                    {portfolio.top_holdings.map((h) => {
                      ...
                      return (
                        <tr key={h.id || h.symbol} className="hover:bg-white/[0.02]">
                          <td className="py-3">
                            <div className="font-semibold text-white">{h.symbol}</div>
                            <div className="text-[10px] text-slate-400 truncate max-w-[120px]">{h.company_name || 'Equity'}</div>
                          </td>
                          ...
                          <td className="py-3 text-right font-mono font-medium text-white">{formatCurrency(h.market_value)}</td>
          ...
          <div className="mt-4 pt-3 border-t border-border-subtle/60 flex items-center justify-between text-xs text-slate-400">
            <span>Net Unrealized Position</span>
            <span className={portfolio.unrealized_gain_loss >= 0 ? 'text-emerald-400 font-semibold font-mono' : 'text-rose-400 font-semibold font-mono'}>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">Latest AI Intelligence</h3>
              </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {portfolio_intelligence.latest.symbols_analyzed.map((sym) => (
                        <span key={sym} className="px-2 py-0.5 rounded-md bg-surface-100 border border-border-strong text-[11px] font-mono text-slate-300">
                          {sym}
                        </span>
                      ))}
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-white">Watchlist Priorities</h3>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400 font-mono">
                {watchlist.high_priority_count} High Priority
              </span>
              <Link to="/app/watchlist" className="text-xs text-cyan-400 hover:text-cyan-300 font-medium">
                Manage
              </Link>
            </div>
          </div>
          ...
          {watchlist.items.length === 0 ? (
            <div className="text-center py-8 text-slate-500 text-xs">
              ...
              <Link to="/app/watchlist" className="text-cyan-400 underline">
                Add securities
              </Link>
            </div>
          ) : (
              ...
              {watchlist.items.map((item) => (
                ...
                >
                  <div>
                    ...
                    {item.notes && <p className="text-[11px] text-slate-400 truncate max-w-xs mt-0.5">{item.notes}</p>}
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-mono font-semibold text-white">{formatCurrency(item.current_price)}</div>
                    {item.price_change_24h_percent !== undefined && item.price_change_24h_percent !== null && (
                      <div className={item.price_change_24h_percent >= 0 ? 'text-[10px] text-emerald-400 font-mono' : 'text-[10px] text-rose-400 font-mono'}>
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/app/alerts" className="text-xs text-amber-400 hover:text-amber-300 font-medium">
                ...
                <div
                  ...
                  className={`p-3 rounded-xl border transition-colors ${
                    !alert.is_read
                      ...
                      : 'bg-surface-100/40 border-border-subtle opacity-75'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-xs text-white">{alert.symbol}</span>
                      <StatusBadge status={alert.severity} size="sm" />
                    </div>
                    <span className="text-[10px] text-slate-500 font-mono">{formatDate(alert.created_at)}</span>
                  </div>
                  <p className="text-xs text-slate-300 line-clamp-1">{alert.title}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/app/research" className="text-xs text-brand-400 hover:text-brand-300">
              ...
              {research.recent.slice(0, 3).map((r) => (
                <div key={r.id} className="p-2.5 rounded-lg bg-surface-100/50 border border-border-subtle text-xs">
                  <div className="font-semibold text-white flex items-center justify-between">
                    <span>{r.symbol}</span>
                    <span className="text-[10px] text-slate-400 font-mono">{formatDate(r.created_at)}</span>
                  </div>
                  <p className="text-slate-400 text-[11px] truncate mt-0.5">{r.summary}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-white">Notification Delivery</h3>
            </div>
            <Link to="/app/notifications" className="text-xs text-emerald-400 hover:text-emerald-300">
              Preferences
            </Link>
          </div>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between py-1 border-b border-border-subtle/50">
              <span className="text-slate-400">Enabled Channels:</span>
            ...
            <div className="flex justify-between py-1 border-b border-border-subtle/50">
              ...
              <span className="text-emerald-400 font-mono font-medium">{notifications.delivered_count}</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-slate-400">Failed / Retrying:</span>
              <span className={notifications.failed_delivery_count > 0 ? 'text-rose-400 font-mono font-medium' : 'text-slate-400 font-mono'}>
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-white">Monitoring Health</h3>
            </div>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between py-1 border-b border-border-subtle/50">
              <span className="text-slate-400">User Alerts Telemetry:</span>
              <span className="text-white font-mono">{monitoring.user_alerts_enabled ? 'Enabled' : 'Disabled'}</span>
            </div>
            {monitoring.latest_run ? (
              <>
                <div className="flex justify-between py-1 border-b border-border-subtle/50">
                  <span className="text-slate-400">Latest Scheduled Cycle:</span>
