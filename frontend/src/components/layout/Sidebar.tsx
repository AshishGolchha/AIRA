  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-500 dark:text-slate-400 -mt-1 font-mono tracking-wider">INVESTMENT AI</span>
              </div>
            )}
          </div>
          <button
            onClick={onToggleCollapse}
            className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1.5 rounded-lg hover:bg-slate-900/5 dark:hover:bg-white/5 transition-colors"
          ...
          {navigationItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isCollapsed && (
                  <div className="absolute left-full ml-3 px-2 py-1 bg-surface-50 dark:bg-surface-100 text-slate-900 dark:text-white text-xs rounded-md shadow-lg border border-border-strong whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50">
                    {item.label}
                  </div>
                )}
              </NavLink>
      ...
      <div className="p-3 border-t border-border-subtle">
        <div className={cn('flex items-center gap-3 p-2 rounded-xl bg-surface-100 dark:bg-surface-200/50 border border-border-subtle', isCollapsed && 'justify-center p-1.5')}>
          ...
          {!isCollapsed && (
            <button
              onClick={logout}
              title="Sign Out"
              className="text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 p-1.5 rounded-lg hover:bg-rose-500/10 transition-colors"
            >
